...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        if self.crossword.overlaps[x, y]:
            ...
            for d1 in self.domains[x].copy():
                consistent = False
                for d2 in self.domains[y]:
                    if d1 != d2 and d1[i] == d2[j]:
                        consistent = True
                if not consistent:
                    self.domains[x].remove(d1)
                    revised = True
        return revised
        ...
        if not arcs:
            ...
            for d1 in self.domains:
                for d2 in self.domains:
                    if d1 != d2 and (d1, d2) not in arcs:
                        arcs.append((d1, d2))
        for d1, d2 in arcs:
            if self.revise(d1, d2):
                for neighbor in self.crossword.neighbors(d1):
                    arcs.append((d1, neighbor))
        return len(self.domains[d1]) > 0
    ...
    def select_unassigned_variable(self, assignment):
        ...
        return values.
        """
        unassigned = list(filter(lambda x: x not in assignment, self.domains))

        unassigned.sort(key=lambda x: len(self.crossword.neighbors(x)),
                        reverse=True)
        unassigned.sort(key=lambda y: len(self.domains[y]))
